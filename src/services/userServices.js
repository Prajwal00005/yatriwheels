...
exports.createMerchant = async data => {
  ...
  if(data.roles.includes["USER","MERCHANT"]) return res.status(400).send("access denied")
...
exports.updateMerchant = async (id, data) => {
  try {
    ...
    throw new Error(error.message || 'Something went wrong while updating the merchant');
  }
};
